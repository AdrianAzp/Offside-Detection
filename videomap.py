...
def get_coordinates(filtro):
    ...
    for i in range(len(contours)):
        cx = int(centers[i][0])
        cy = int(centers[i][1])
        if int(radius[i]) < 10 and cx > 100 and cy > 60 and cx < 1100:  # ESTE < 600 SIRVE PARA ELIMINAR PUNTOS QUE ESTÉN FUERA DEL CAMPO Y SEAN INUTILES
...
cap = cv.VideoCapture('./videos/offside5.mp4')
...
while cap.isOpened():
    ...
    ret, src = cap.read()
    if ret:
        plan_view, lista_teamA, lista_teamB = get_plan_view(src, dst)
        plan_view = merge_views(src, dst)
        dst = paint_points(dst)
        cv.imshow('Homografia', plan_view)
        cv.imshow('Frame', dst)  # Display the resulting frame
        dst = cv.imread('./images/campo.png', -1)
        if cv.waitKey(1) & 0xFF == 27:  # Press ESC on keyboard to exit
            break
    else:  # Break the loop
        break
